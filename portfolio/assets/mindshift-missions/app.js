...
function exportPDF() {
    ...
    const SECTION_BG = [245, 246, 248];    // light gray card fill
    const SECTION_BORDER = [215, 220, 228]; // subtle border
    const TITLE_COLOR = [177, 80, 67];     // #b15043 terracotta
    const LABEL_COLOR = [110, 118, 130];   // medium gray labels
    const TEXT_DARK = [30, 35, 48];        // near-black body text
    ...
    const drawPageBackground = (pdf) => {
        // Page background
        pdf.setFillColor(...hexToRGB(BG_COLOR));
    ...
    const drawTeamChip = (pdf, text, x, y, maxWidth) => {
        const chipH = 4, chipPadding = 3, fontSize = 7;
        pdf.setFontSize(fontSize);

        const textWidth = pdf.getTextWidth(text.toLowerCase());
        ...
        pdf.text(text.toLowerCase(), x + chipPadding, y + chipH - 1);
    ...
    const drawBadgePill = (pdf, text, x, y, maxWidth) => {
        ...
        const badge = '★ ' + text;
        const textWidth = pdf.getTextWidth(badge.toLowerCase());
        const chipW = textWidth + chipPadding * 2;

        if (x + chipW > maxWidth) return null; // badge doesn't fit

        // Badge background (light terracotta tint)
        pdf.setFillColor(248, 235, 232);
        pdf.setLineWidth(0.2);
        pdf.setDrawColor(200, 140, 130);
        pdf.roundedRect(x, y, chipW, chipH, 1, 1, 'FD');

        // Text
        pdf.setTextColor(150, 60, 50);
        pdf.text(badge.toLowerCase(), x + chipPadding, y + chipH - 1);
    ...
    const drawCompactLV = (pdf, label, value, x, y, w, maxLines = 2) => {
        const fs = 6.5, lh = fs / 2;
        pdf.setFontSize(fs);
        pdf.setFont('Helvetica', 'bold');
        pdf.setTextColor(...LABEL_COLOR);
        pdf.text(label.toLowerCase() + ':', x, y);
        y += lh + 0.5;
        pdf.setFont('Helvetica', 'normal');
        pdf.setTextColor(...TEXT_DARK);
        const lines = pdf.splitTextToSize(value.toLowerCase(), w).slice(0, maxLines);
        pdf.text(lines, x, y, { maxWidth: w });
        return y + lines.length * lh + 1.5;
    };
    ...
    const COL2_X = MARGIN_LEFT + COL_W + GAP;
    const PAD = 2;
    const CTW = COL_W - PAD * 2;   // card text width per column
    const FTW = CONTENT_W - PAD * 2; // full-width text width

    // --- ROW 1: Mission Overview (left) + Outcome Snapshot (right) ---
    pdf.setFontSize(9);
    pdf.setFont('Helvetica', 'bold');
    pdf.setTextColor(...TITLE_COLOR);
    pdf.text('mission overview', COL1_X, currentY);
    pdf.text('outcome snapshot', COL2_X, currentY);
    currentY += 3;

    // Pre-calc Mission Overview height
    pdf.setFontSize(7);
    const _lh7 = 7 / 1.8;
    let moH = 3;
    let _tx = 0, _cr = 1;
    for (const name of teamNames) {
        ...
        if (_tx + tw > CTW) { _cr++; _tx = 0; }
        _tx += tw + 2;
    }
    moH += _cr * 5 + 1;
    const _rlns = pdf.splitTextToSize((gameState.selectedRole || 'not selected').toLowerCase(), CTW);
    const _slns = pdf.splitTextToSize(scenario.title.toLowerCase(), CTW);
    moH += (_lh7 + 1 + Math.min(_rlns.length, 2) * _lh7 + 3);
    moH += (_lh7 + 1 + Math.min(_slns.length, 2) * _lh7 + 3);
    moH += (_lh7 + 1 + _lh7 + 3);
    moH += 1;

    // Pre-calc Outcome Snapshot height
    let osH = 2;
    [outcomeVal1, outcomes.public_perception, outcomes.operational].forEach(val => {
        const lns = pdf.splitTextToSize(val.toLowerCase(), CTW);
        osH += (_lh7 + 1 + Math.min(lns.length, 3) * _lh7 + 3);
    });
    osH += 1;

    const row1H = Math.max(moH, osH);
    drawCardBackground(pdf, COL1_X, currentY, COL_W, row1H);
    drawCardBackground(pdf, COL2_X, currentY, COL_W, row1H);

    // Fill Mission Overview
    let moY = currentY + 2;
    pdf.setFontSize(7);
    pdf.setFont('Helvetica', 'bold');
    pdf.setTextColor(...LABEL_COLOR);
    pdf.text('team members:', COL1_X + PAD, moY);
    moY += 3;

    let chipX = COL1_X + PAD;
    const maxChipX = COL1_X + COL_W - PAD;
    for (const name of teamNames) {
        const nx = drawTeamChip(pdf, name, chipX, moY, maxChipX);
        if (!nx) {
            moY += 5;
            chipX = COL1_X + PAD;
            const ax = drawTeamChip(pdf, name, chipX, moY, maxChipX);
            chipX = ax || (COL1_X + PAD);
        } else {
            chipX = nx;
        }
    ...
    moY += 6;
    moY = drawLabelValue(pdf, 'role', gameState.selectedRole || 'not selected', COL1_X + PAD, moY, CTW, 7, 2);
    moY = drawLabelValue(pdf, 'scenario', scenario.title, COL1_X + PAD, moY, CTW, 7, 2);
    moY = drawLabelValue(pdf, 'total time', formatTime(Date.now() - gameState.startTime), COL1_X + PAD, moY, CTW, 7, 1);

    // Fill Outcome Snapshot
    let osY = currentY + 2;
    osY = drawLabelValue(pdf, outcomeKey1, outcomeVal1, COL2_X + PAD, osY, CTW, 7, 3);
    osY = drawLabelValue(pdf, 'public perception', outcomes.public_perception, COL2_X + PAD, osY, CTW, 7, 3);
    osY = drawLabelValue(pdf, 'operational', outcomes.operational, COL2_X + PAD, osY, CTW, 7, 3);

    currentY += row1H + 4;

    // --- ROW 2: Checkpoint 1 (left) + Checkpoint 2 (right) ---
    if (!canFitOnPage(80)) newPage();

    const cp1 = scenario.checkpoints[1];
    const r1 = gameState.responses.checkpoint1;
    const cp1Fields = [
        ['situation', cp1.title],
        ['question', cp1.questions[r1.question] || 'none'],
    ...
    const cp2Fields = [
        ['situation', cp2.title],
        ['question', cp2.questions[r2.question] || 'none'],
    ...
    pdf.setFont('Helvetica', 'bold');
    pdf.setTextColor(...TITLE_COLOR);
    pdf.text('checkpoint 1', COL1_X, currentY);
    pdf.text('checkpoint 2', COL2_X, currentY);
    currentY += 3;

    // Pre-calc checkpoint heights (compact)
    const cpFS = 6.5, cpLH = cpFS / 2;
    pdf.setFontSize(cpFS);
    let cp1H = 2;
    for (const [, val] of cp1Fields) {
        const lns = pdf.splitTextToSize(val.toLowerCase(), CTW);
        cp1H += (cpLH + 0.5 + Math.min(lns.length, 3) * cpLH + 1.5);
    }
    cp1H += 1;
    ...
    for (const [, val] of cp2Fields) {
        const lns = pdf.splitTextToSize(val.toLowerCase(), CTW);
        cp2H += (cpLH + 0.5 + Math.min(lns.length, 3) * cpLH + 1.5);
    }
    cp2H += 1;

    const row2H = Math.max(cp1H, cp2H);
    drawCardBackground(pdf, COL1_X, currentY, COL_W, row2H);
    drawCardBackground(pdf, COL2_X, currentY, COL_W, row2H);

    // Fill Checkpoint 1 (compact)
    let c1Y = currentY + 1;
    for (const [label, val] of cp1Fields) {
        c1Y = drawCompactLV(pdf, label, val, COL1_X + PAD, c1Y, CTW, 3);
    }

    // Fill Checkpoint 2 (compact)
    let c2Y = currentY + 1;
    for (const [label, val] of cp2Fields) {
        c2Y = drawCompactLV(pdf, label, val, COL2_X + PAD, c2Y, CTW, 3);
    }
    ...
    pdf.setFontSize(7);
    const _r1lns = pdf.splitTextToSize(refl1.toLowerCase(), FTW);
    const _r2lns = pdf.splitTextToSize(refl2.toLowerCase(), FTW);
    let reflH = 2 + (_lh7 + 1 + Math.min(_r1lns.length, 4) * _lh7 + 3)
              + (_lh7 + 1 + Math.min(_r2lns.length, 4) * _lh7 + 3) + 1;

    drawCardBackground(pdf, MARGIN_LEFT, currentY, CONTENT_W, reflH);

    let rfY = currentY + 2;
    rfY = drawLabelValue(pdf, scenario.reflection_prompts[0], refl1, MARGIN_LEFT + PAD, rfY, FTW, 7, 4);
    rfY = drawLabelValue(pdf, scenario.reflection_prompts[1], refl2, MARGIN_LEFT + PAD, rfY, FTW, 7, 4);

    currentY += reflH + 3;

    // --- Badges ---
    if (gameState.earnedBadges && gameState.earnedBadges.length > 0) {
        if (!canFitOnPage(12)) newPage();
    ...
    // --- Logos (positioned dynamically below content) ---
    const totalPages = pdf.internal.getNumberOfPages();
    pdf.setPage(totalPages);

    const logoY = Math.max(currentY + 2, A4_H - 20);
    const prmeW = 20, prmeH = 10.5;
    const prmeX = A4_W - 54;
    pdf.addImage(PRME_LOGO_B64, 'PNG', prmeX, logoY, prmeW, prmeH);

    pdf.setFontSize(9);
    pdf.setTextColor(180, 180, 180);
    pdf.text('\u00d7', prmeX + prmeW + 2, logoY + prmeH / 2 + 1.5);

    const wvW = 24, wvH = 12.6;
    pdf.addImage(WV_LOGO_B64, 'PNG', prmeX + prmeW + 6, logoY - 1, wvW, wvH);
